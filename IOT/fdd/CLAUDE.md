...
17. 制冷模式外机 EEV 满开(非伺服,量产代表性控制;试制机 u31×A 工况见隔离标注):exv_resid 在制冷侧无泄漏信息量,禁止作为制冷泄漏证据;其语义 = EEV 卡滞/控制异常指示(满开偏离)。制冷泄漏载体 = sc_resid(早期主通道)+ capacity_resid(佐证,非硬门)/ sh_resid(中晚期,无需 exv_saturated 门——TXV 不可见,Sh 抬升本身即节流件失权信号)。制冷 sh_resid 参照面 = 逐机×同箱,跨机/全局参照禁止(实测同箱跨机散差 ~9K,结构性)。与铁律 3(制热 Sh 钉零)成镜像:两模式各失一通道,制冷侧无可见执行器前兆通道。
...
- **[SUPERSEDED by FDD-I-016(字母层与 condition_of 签名已被取代;容差/同温歧义/模式区分规则仍现行)]** 工况命名对齐 AHRI 210/240(FDD-I-015):项目旧名→AHRI 标准名 H1→H1N、H2→H22、H3→AHRI H4(−8.3)、H4→H4Full(−15);**关键**:项目旧"H4"是 −15℃ = AHRI **H4Full**,AHRI 的 H4 是 −8.3℃(项目无该温区数据)。代码工况标签统一 AHRI 标准名。判定 = condition_of(Ta, mode),容差 ±1.5℃(AHRI 干球 ±0.56 + 记录 ±1,算术和);同温同模式(A/A2@35、C/D@19.4)Ta 不可分→标 A_or_A2/C_or_D(频率/启停辅助判定下批);H12@19.4 制热 vs C/D@19.4 制冷靠模式区分。工况点/容差在 config/calibration.yaml。(字母层与签名已由 FDD-I-016 再对齐 2026 版,见下下条。)
- 2436AA 健康工况实测(FDD-I-015 item4,排除 31 号 5 少冷媒文件):H22(旧 H2)健康锚=0——此前"H2 覆盖"几乎全是 2 个少冷媒文件(FDD-I-014 证 H2 99% 欠充);低温制热落 H4Full(−15,24 frosting_steady)非 AHRI H4(−8.3,无数据)。31 号 H2/H22 sense 自检红为该欠充污染在 sense 层的体现(非代码 bug;FDD-I-017 已文件级重标 fault_injected,该红已消)。
- 工况字母定名 2026 版 + 容量轴(FDD-I-016,测试即规格 tests/test_m2_condition_of.py):按 AHRI 210/240-2026 Table 8 重命名 H1N→H1、H22→H2、H4(−8.3)→H3、H4Full→H4、H12→NONSTD_HEAT,新注册 H0(16.7,当前无数据);NONSTD_HEAT=19.4℃(67°F)制热实验室验证点、非 AHRI 标准,condition_class=extreme 不计额定覆盖(锚/envelope/清单细则下游批次)。台架元数据名(2023-M1 命名)经 c4.AHRI2026_NAME 归一,认证窗与 surrogate 窗回到单一命名面;实测迁移为纯双射(行/锚计数逐行不变,覆盖门两 SKU 保持 4/4)。condition_of 契约升级 (Ta, mode, freq)→(letter, capacity, confidence):字母归属 = ±1.5℃ 内最近工况中心(仅裁决 H0/NONSTD 0.3℃ 重叠,非重叠点等价落带);容量轴与字母正交,全负荷频率带 lab InvHz 制冷 [66,76]/制热 [70,86] Hz 闭区间(config capacity.*),带外(含超上界,不自扩带)与缺失均 UNKNOWN_CAPACITY;现场 CompRps 阈值另标定(M3)。sense 预存红签名随迁移 'H22'→'H2'(同一欠充污染,仍恰 1 红)。
- 31 号少冷媒重标落地 + 数据面双层澄清(FDD-I-017):5 个 2023-12-09 少冷媒文件(094220/112220/130220/144220 各 5990 行、162220 4730 行,1s 原始)经 config/unit_sku_map.yaml `file_data_type` 文件级分流标 fault_injected(不解析 severity 档位,命名规范三项未闭);路由唯一断点 c4._apply_data_type_routing 在清锚外同步中性化 condition_class(rating→fault_injected)——sense/覆盖/瞬态/包络全部 rating 类消费者经同一分流排除,行保留加载(144220→H2 202 行,162220→H1 216 + H2 200 行,10s 面,锚全 0;另 3 文件从未产 surrogate 窗)。sense v3 预存红随之消:M2 = 49 passed + 0 failed + 3 skipped,覆盖门两 SKU 仍 4/4。FDD-I-015 item4 数表口径考定 = **仅 31 号机、1s 面、clean/frost 锚行计数**:v2 下逐格双射复现(A_or_A2 12100/0、H1 3986/0、B 3139/0、H4 0/24、H2 0/0 整字母消失、UNKNOWN frost 54;UNKNOWN clean 94841+94=94935,少的 94 行被新注册 H0 吸走 [15.2,17.9) 段;旧 H12 37 clean 行 = H0 5 + NONSTD_HEAT 32,总 p50 18.27)。分类双平面成文:**窗面**(台架元数据窗经 _reclassify + surrogate 窗经 _resolved_condition → test_condition/condition_class 列,供额定锚池、覆盖门、envelope 输入)与**行面**(condition_of 逐行 → ahri_condition/condition_confidence,只供 uncertainty_report 与审计),现无混用;M3 现场数据无窗面、行面将成唯一平面,归一时点须先裁。容量带实测:H4(−15)全负荷锚频 94–106 Hz(anchors 856 行全部 >86,全行 2038/2336 >86)越 heat 带上界,带不自扩、等人工调带;H1 锚 100% Full(2436AA 全池 12584/12584,InvHz 76–84)。翻译表外枚举:载入面未注册值仅 8 行(−125/−1/14/121),原始面审计另见 12(2135 行)/8/65/66/−16/−101(窗外,入厂商清单);铁律 15 "5%" 的分母口径(全部隔离含已注册停机族 vs 仅表外未注册)待裁。
- envelope_input/rating_anchor 拆分 + 2436AA 干净重拟合(FDD-I-018,测试即规格 tests/test_m2_envelope_input.py 6 绿):载入面新增独立布尔列 envelope_input(c4._derive_envelope_input,紧随 _apply_data_type_routing 同一 choke point):rating_anchor 供额定覆盖门/O-CERT,NONSTD_HEAT 行强制 False(规则①;anchor_type 保留——稳态是 seg 层物理事实,不随命名清除);envelope_input 供 L1 拟合 = 健康 × 稳态锚 ×(rating_anchor ∨ NONSTD)(规则②,延展制热高 Ta 端);fault_injected 两列同 False(FDD-I-017 排除保持);baseline.fit_envelope 入口按 envelope_input 内置行选择(幂等)。实测:现有载入数据上两列恒等(NONSTD 窗=0——19.4℃ 制热段仅约 2.5min<surrogate 平台门 600s 铸不成窗,规则②在载入面休眠,语义由合成三类行测试钉死);2436AA 污染修复:H1 锚 490→342 行、预测 Qh(H1) 11.223→11.676 kW(+4.0%)、制热 Ta 斜率 +0.0576→+0.3976 kW/K(污染曾把斜率压平约 7 倍);H2 健康锚仅 1 行(84 号),预测 9.329 kW 单行支撑、数据补充前脆弱;4860AA dirty==clean 逐条全等(未受污染)。物理合理性 DoD 实测:单调性符号双 SKU 双模式全对(制热 +0.3976/+0.2840,制冷 −0.0253/0),相邻工况比 1.252/0.982/1.458/1.116 全在 [0.3,3.0],病态外推 False;留一 MAPE 仍 M3(holdout 保持 skip)。
- DK-016(FDD-I-019-R1 裁决 1):u31 试制机 × A 工况 EEV 部分开度(Exv 286–316,稳态 104 行/载入 197 行,3 文件 2023-11-23 172117/205133 + 2023-11-27 105005)——机台级异常,隔离标注(config unit_sku_map `cooling_ref_quarantine` → c4 物化布尔列,行不删除,留作 exv 异常通道 DK-009-d 首个真实样本);其余 5 组机台×工况 Exv 恰 480 严格满开,DK-009 收窄为"量产代表性控制下满开非伺服"维持;机理进 O-VENDOR 问题包(组织侧)。
- DK-017(FDD-I-019-R1 裁决 2):制冷 sh 跨机同箱散差 9.3K(u31 B 0.75 vs u55 B 10.02,同为 Exv=480 满开),逐机×工况内 std ≤0.74K——制冷 sh_resid 参照面 = 逐机×同箱(unit × mode × Ta 2K × 频率档),跨机/SKU 级参照禁止(全局带被数据证伪);SH_NORM_BAND_COOL=3.0 维持、限定逐机参照面(config diag.sh_norm_band_cool 注 plane: per_unit_bin,≈4σ 保守,M4 UC 梯度重标)。1b 复测(排除 u31×A 后逐机×同箱):2436AA P95=1.120K、4860AA 0.085K,门 2.0K 过。
- DK-018(FDD-I-019-R1 裁决 3):dsh_phys(=Td−tc_sat)− HDSH 为跨管线口径差非误差(固件独立滤波/采样相位 vs 10s 中值面):稳态 P95 0.736K、无系统偏置(全体 signed mean −0.074K);原 0.2K DoD 系规格错配(把饱和表静态精度 ±0.1K 误当跨管线一致性上限,方案侧认领)——重定义为 双门:全体 |bias|≤0.3K ∧ 稳态 P95≤1.5K(实测 0.074/0.736 双过,dsh_phys 物化放行);逐机 P95 入档:31 号 0.662 / 44 号 0.834 / 55 号 0.731 / 84 号 0.353 / 85 号 0.196。dsh_phys 用途 = 注释/安全门(DK-013 四允许一禁止、DK-015 非硬门),K 量级精度即够。
