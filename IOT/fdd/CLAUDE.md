...
- M3 大批量禁用逐行 PropsSI(慢 + native segfault 风险,几十亿行必反复触发);改预计算 R410A 饱和查表(1.0–46 bar,露点/泡点各一张)+ 线性插值,无 native 崩溃、可复现。此为 M3 前置技术项,M2 期消化;查表口径恒等于 CoolProp(离线预计算),插值精度 DoD <0.01K,封板 sh_phys/sc_phys 数值不移。
