...
- **题库 40 → 85 题**(编写源 [js/quiz-bank.js](js/quiz-bank.js) → 构建生成 [bank/](bank/) 分片),52 个误区节点:
  - 补齐 EPA 608 正规结构:新增 **Type II 独立板块**(高压系统,最常考)6 题;core 4→8、type1 4→7、type3 4→7(回收 80/90%、抽空 in Hg vs 微米、销售限制、记录义务、系统依赖/自足回收设备等)
  - A2L 9→14、rule26 3→7(温度滑移 bubble/dew、SNAP 端用限定、AIM 双杠杆=配额削减 vs GWP 限值、AHRI 700 reclaim)
  - 州执照机械规范四板块各 4→7(排气分类/plenum 材料/grease duct、单开口燃烧空气 1/3000、烟道衬里与冷凝腐蚀)
  - **新增 NATE Core 板块** 8 题(按官方 KATE 结构:欧姆定律/串并联=电气占比最大、万用表档位、传热三模式+显/潜热、LOTO+PPE、气流静压~400cfm/ton、软技能)
- **抽样上限**([js/quiz.js](js/quiz.js)):板块 9→11,每板块抽 2 题=22 候选,新增 `TARGET_N=18` 随机裁剪(待复习错题优先保留不被裁),单局稳定 ~7 分钟;i18n 加 type2/nate 中英标签(NATE 品牌名保留英文)
- **内容红线**:机械规范题仅原创复述通识概念与广泛采用经验数值,不摘 IMC 条文原文;NATE 板块标注非官方练习;各州采用版本不同,页面已有免责说明
- **浏览器全回归**:12 分片全部 200、单局稳定裁到 18 题、11 板块柱状图、EN/中板块标签正确、错题本不回归、零控制台报错;构建脚本零错误零告警(52 误区节点无孤儿、无题干重复)
